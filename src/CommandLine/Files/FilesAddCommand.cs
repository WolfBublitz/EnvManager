...
internal sealed class FilesAddCommand : BaseCommand
{
    ...
    protected override async Task ExecuteAsync(ParseResult parseResult, CancellationToken cancellationToken)
    {
        ...
        await AnsiConsole.Status().StartAsync($"Adding {files.Count()} file(s)...", async ctx =>
        {
            StringBuilder messageBuilder = new();
            messageBuilder.AppendLine($"Added {files.Count()} file(s):");
